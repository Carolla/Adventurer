...
public class SplashInitWindow extends JDialog 
{
	...
	private static final String SPLASH_PIC = "Adv_UserExperience/images/ChronosLogo.jpg";    	
	/** Splash screen will display this long initally before checking for end of background processing. */
	private final int TIMER_INITIAL_DELAY = 9000; 			// minimum time for the splash window to show (in milliseconds)  
...
//        setPreferredSize(new Dimension(adjSplashWidth, adjSplashHeight));
        System.err.println("Splash size (width, height)= " + adjSplashWidth + ", " + adjSplashHeight);
...
//	            		Dgn.debugMsg("Timer returned but processing still in play." + System.currentTimeMillis());
        ...
        //  Returns initially after this amount of time
        _timer.setInitialDelay(TIMER_INITIAL_DELAY);			
        _timer.start();
//        Dgn.auditMsg("Timer started in splash window.");
        System.out.println("Timer started in splash window.");
        //  Checks periodically if this task is completed
        runBackgroundTask();					
...
//        Dgn.auditMsg("Thread invoked background task run method.");
        System.out.println("Thread invoked background task run method.");
...
//        Dgn.debugMsg("SplashInitWindow.timedOut(): initializaton complete.");
        System.out.println("SplashInitWindow.timedOut(): initializaton complete.");
    	_timer.stop();
        setVisible(false);
    }
