...
/**
 * Creates a non-modal singleton window to display help information. This object has
 * only one real interface method:
 * <p>
 * ==>&nbsp&nbsp&nbsp <code>help(String helpID)</code> to display the text associated with the Help ID when the Help key
 * is pressed (or menu item selected).
 * <p>
 * The text displayed depends on the focus object, and changes accordingly. This window has a lock so that help messages
 * are written only once even if the <code>HelpDialog</code> is called multiple times.
 * <p>
 * The first time <code>HelpDialog</code> is opened (i.e., the singleton is created), it creates the PDC
 * <code>HelpEngine</code> to coordinate between this HIC class and the <code>HelpReader</code> DMC class.
 * <p>
 * Implementation Note: Applications may hook into the Help library objects by defining an actionListener that calls the
 * <code>HelpDialog</code> constructor in its <code>actionPerformed()</code> method. The application must pass a
 * directory name containing the help files for the application, and the name of the XML text file. The
 * <code>HelpEngine</code> will do all the rest: It will create the <code>HelpReader</code> that will convert the XML
 * text file to a binary indexed random access file (named after the xml filename, but with a <i>.hlp</i> extension),
 * place it in the target directory, and manage the retrieval of help text based on the helpID requested.
 * 
 * @author Alan Cline
 * @version <DL>
 *          <DT>1.0 Nov 22 2008 // adapted from older program <DD> <DT>1.1 Dec 13 2008 // adapted for this program <DD>
 *          <DT>2.0 July 9 2009 // refactored to make more application-independent <DD> <DT>2.1 Aug 18 2009 // minor
 *          revisions to accommodate other class changes<DD> <DT>3.0 Nov 23 2013 // converted from XML input files to
 *          Registry database
 *          </DL>
 */
public class HelpDialog extends JDialog
{
    ...
    /**
     * Allow (false) or disallow (true) text to be written to the window; defaults to <code>true</code> to remind
     * programmer to unlock it before writing, and then relock
     * it if desired.
     */
    ...
    /**
     * Create the singleton HelpDialog if it doesn't already exist; pass the parms on
     * to the private constructor. HelpEngine expects a Registry to hold the help text
     * 
    ...
    /**
     * Retrieves and displays help text from the Help file for the given key
     * associated with the widget for which the Help was requested, either by the F1 key or
     * the Help MenuItem. The <code>HelpDialog</code> window is non-modal and the
     * text within is replaced with the appropriate text for the widget under focus. <br>
     * If there is no associated widget, the help window is displayed for the owner (frame or
     * panel), and should contain general information.
     * 
     * @param helpID the key associated with the widget or panel that called the Help,
     *            and indicates which msg to display from the Help file
...
//        }
    ...
    /**
     * Clears the contents of the HelpWindow so new content can be added.
     * Respects the lock flag; if Lock is true, does not clear this window.
     */
    public void clear()
    {
        _lockflag = false;
    ...
    /**
     * Overrides the <code> JTextArea</code> append method so that <code>HelpDialog</code> can lock data from being
     * written to it.
    ...
    /**
     * Overloads the component insert method to ensure that text added is shown from
     * the top down, and not shown only at the bottom.
     * If Lock is true, allow no more writing to this window
     * 
     * @param msg Whatever text is to be written in the window
     */
