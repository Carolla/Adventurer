...
/**
 * Creates and opens the db4o database for object persistence. 
 * The database can be set for read-write or read-only mode after it is opened.
 * Handles all object persistence operations and database management.
 * If the database is closed, it will be nulled out, but the DBRegistryReadWriter will still exist.
 * 
 * @author Alan Cline
 * @version <DL>
 *  <DT>Build 1.0 Aug 12, 2012 // original <DD>
 *  <DT>Build 1.1 Dec 16, 2012 // updated with more robust testing <DD>
 *  <DT>Build 2.0 Feb 25 2013   // replaced queryByExample with native queries  <DD>
 *  <DT>Build 2.1 Mar 18, 2013     // revised after adding IRegistryElement 
 *  </DL>
 */
public class DbReadWriter
{
    ...
    /**
     * Add a new object into the database. All abnormal cases that
     * cause exceptions to be thrown must be handled by the caller
     * 
 ...
 //           _db = null;
    ...
    /**
     * Verify if a particular object exists, found by calling that objects <code>equals()</code> method
     * 
    ...
    /**
     * Delete an object from the database. The object must be retrieved before being deleted.
     * The caller must handle exceptions if the database is closed or Read-Only.
     * 
    ...
    /** Create the object container for transaction processing with the default configuration;
     * set the db to open status.
     * This is used when the DBRW is created.<P>
     * NOTE: The folder structure must exist before a db file within it can be created. 
     * db4o will not create folders: db4o will throw an enigmatic System IO error. 
     *
    ...
    {
        try {
            // Open the db only if it is not already open. The file is created or reloads the ObjectContainer
            if (_db == null) {
    ...
    /**
     * Set the database to ReadOnly (true) or ReadWrite (false), depending on the parm.
     * Reset the current configuration as indicated, close the database, and open it with the
     * new requested configuration. 
     * 
     ...
     * Add a new object into the database. All abnormal cases that
     * cause exceptions to be thrown must be handled by the caller.
     * This is same as <code>dbAdd()</code> except that it takes a generic Object instead of a
     * <code>IRegistryElement</code> object.
     * 
     * @param obj          object to add
     * @throws NullPointerException                 registry should not try to save a null
     * @throws DatabaseClosedException        db needs to be in correct state  
    ...
    public class MockDBRW
    {
        ...
        /** Deletes all elements in the registry,  its ObjectContainer, and deletes the associated db file.
         * This method is only used for testing. <p>
