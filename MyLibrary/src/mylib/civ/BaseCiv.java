...
import mylib.pdc.ObservableModel;

/**
 *  An abstract base class that contains common functions for all Civ classes,
 *  and supports the MVP (Model-View-Presenter) Stack.
 *  This class takes a DataShuttle of ModelKeys M, and a DataShuttle of WidgetFields W.
 *
 * @author Alan Cline
 * @version <DL>
 * <DT> Build 1.0       May 28, 2011   // original <DD>
 * <DT> Build 1.1       Jul 29 2011   // update after generics working <DD>
 * </DL>
 */
public abstract class BaseCiv<M extends Enum<M>, W extends Enum<W>> 
            implements Observer
{
     ...
     * @return shuttle containing String data data for the model keys; 
     *          shuttle's error flags are set on error
     */
    ...
    /** Creates and loads a shuttle with the widget's default field values.
     * The default data can come from fields in the Civ, or from the model, 
     * or from another class that has default data to display.
     *  
...
//    }
    ...
    /** Default method to be overwritten if application needs to do anything. 
     * Called by submit() method after validation and conversion of widget to data shutte,
     * but before the model unpacks the data shuttle. This method can be used to create
     * the model if it doesn't already exist. 
     * 
    ...
    /** Validates the data and reformats it for the PDC object.
     * This method validates the data in the widget shuttle, converts it to model shuttle data,
     * and calls the model's <code>unloadShuttle()</code> method to store the data.
     * Occasionally, the application-level Civ must perform some action on the data shuttle before it is
     * sent to the model, so <code>localAction()</code> method is called. 
     * In most cases, <code>localAction()</code> defaults to the do-noting method in this class.
     * 
...
//    /** Converts the model's data shuttle to a widget shuttle whenever it is updated, and sends it to 
//     * the widget to refresh the GUI.
//    *
//    * @param model  the object being watched for changes
//    * @param objModel arbitary object (should be a DataShuttle<M>) that the model passes to this observer
//    */
//    @Override
