...
/**
 *	A custom random generator biased toward rolling dice for mapping to standard d20 gaming 
 * system rules, including Chronos.  <br>
 * <code>MetaDie</code> has three core methods: 
 * <UL>
 * <LI><code>getRandom(int min, int max)</code>, which selects a single number from a linear 
 * random population within the range, defined by the given min, max limits (inclusive). </LI>
 * <LI> <code>getGaussian(double mean, double lowEnd, double highEnd)</code>, which retrieves 
 * a number from a Gaussian distribution adjusted to fit the given mean. </LI>
 ...
 * </UL>
 *	<code>MetaDie</code> can be created either in debug mode (repeatable numeric sequences) or 
 * with the standard randomizer (non-repeatable). <br>
 * 
 *
 *	@author Alan Cline 
 * @version <DL>
 * <DT> Build 6.0		Dec 10 2006	// re-used from previous programs <DD>
 * <DT> Build 6.1		Apr 28 2009   	// major refactoring to simplify method set <DD>
 * </DL>
...
public class MetaDie 
{
	...
	/**
	*	Creates a Random class for single die throws, using time as the seed.
	...
	*/
	...
	/**
	*	Creates a Random class using an input value as a seed. This object 
	...
	*/
    ...
    /** Returns a Gaussian value within the given range centered about the given average.
     * Numbers outside the low and high ends are excluded. 
     * <p>
     ...
     * standard constants, the call is <code>getGaussian(1.0, 1.0-SIGMA, 1.0+SIGMA)</code>. 
     * Dissimilar <code>lowEnd</code> and <code>highEnd</code> values will alllow asymmetrical 
     * distributions, such as Poisson distributions.
    ...
    /**
 	* Returns a single random number that falls within the linear range requested. 
 	* Ranges represent linear distributions with inclusive boundaries, must be positive, and the 
 	* maxRange must be greater than the minRange.
 	*
    ...
    * from a normal (Gaussian) population that has the given dice characteristics.  
    * If nbrDice = 1, then this method returns a (linear) number from 1 to nbrSides.
    ...
    */
	...
	/**
	* 	Convert a dice notation string to two numbers, then roll the dice as indicated by the string. <br>
...
//       if (cs.length == 3) {
//           String added = cs[2];
//           System.err.println("\t roll(): notation = " + notation + " yields\t " + prefix + "\t" + suffix + "\t" + added);
//       }
//       else {
...
//    }
    ...
    /** Returns a linear random number in terms of percent. 
     * Same as <code>getRandom(1, 100)</code>.
     * 
    ...
    /** Returns a single trait using the 4d6-1 algorithm: Four d6 dice are rolled, and the lowest is
     * removed from the list, giving an average of 11.5 for a Person's trait. Imposing the constraint
     * that all traits must be greater than a certain minimum value.
     * 
     * @return value from 3 (four 1's - 1) to 23 (four 6's - 1) 
     */
...
public class MockMetaDie
{
    ...
    /** Created to catch exceptions easier. The test case contains a series of bad methods
    ...
    *   @param  high      must be (0.0, 1.0]
    ...
    /** Created to catch exceptions easier. The test case class a series of bad methods
     * that throw a lot of exceptions. 
     *  
    *   @param  nbrDice     number of rolls to sum
    *   @param  nbrSides        max range of each roll (number of sides of die)
