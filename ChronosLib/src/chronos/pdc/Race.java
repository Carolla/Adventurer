...
/**
 * Defines the common methods and attributes for all Races; <code>Human</code> is the default Race.
 * A concrete derived subclass of <code>Race</code> is one of the major components of the Person
 * object. <br>
 * NOTE: This class is serializable, so contains transients and statics that will not be stored.
 * 
 * @author Alan Cline
 * @version Feb 14 2009 // original <br>
 *          Feb 28 2009 // modifed after Race and Klass re-design <br>
 *          Mar 5 2009 // modifed for serialization <br>
 *          Mar 15 2009 // removed ctor and added gender accessor <br>
 *          Oct 9 2009 // add satiety attributes <br>
 *          Jan 18 2010 // add Dwarf support <br>
 *          Jul 5 2010 // support for CIV and data shuttle <br>
 *          Sep 26 2010 // modified slightly for QA tag suport <br>
 *          Nov 14 2010 // pulled out any methods that were not part of creating a Person and are
 *          part of running the Person during the game, e.g, Hunger and Age adjustments <br>
 *          May 15 2011 // TAA reinserted skill methods, fixed errors <br>
 *          March 25 2013 // TAA added <br>
 *          September 4 2011 // abc cleaned up comments, minor refactoring
 */
public abstract class Race implements Serializable
{
  ...
  /**
   * Define the kinds of races a Hero might be (first one is default): <br> 
   * Human, Dwarf, Elf, Gnome, Half-Elf, Half-Orc, and Hobbit.
   */
  static public final String[] _races = {
  ...
  /**
   * Each race (except Human) has a race language; half-breed races, like half-elf, have a 50%
   * chance at a racial language.
   */
  ...
  /**
   * Distance (feet) that this Person can see in the dark using infrared vision
   */
  ...
  /**
   * Adjustment to chance To Hit with a missile weapon (speed of aim), based on DEX
   */
  protected int _toHitMissileMod = Constants.UNASSIGNED;

  /**
   * Reference to random generator (MetaDie object) for some of the calculations
   */
   ...
   * Dungeon, particularly after Guild graduation and during promotion. The current age of the
   * Person is initialized in units of seconds.
   * 
  ...
  /**
   * Verify that the specified traits stay within their race limits, else adjust errant values
   * 
  ...
  /**
   * Race is an abstract class, and all subclasses must extend Race: Human, Dwarf, Elf, Gnome,
  ...
  /**
   * Creates a specific subclass of Race based on its class name (String). This method is the Race
   * object factory.
   * 
  ...
  /**
   * Adjust the Prime Traits based on gender, regardless of race. Gender affects Race-based weight
   * and height, so is included in the Race class. <br>
   * Algorithm: Males are default, so no changes are made to the traits. Females are given CON+1,
   * CHR +1, STR-1
   * 
   * @param traitList the six traits will be overwritten with the new ones
   ...
   * 
   * @param skillList target list to which this method will add new skills
  ...
  /**
   * This is called by the subclass, so that the age reflects the particular race; adjustments are
   * based on the gender, age category, and verified against permitted Racial limits.
   * 
   * @param ageCat adjust Person by which age-category they are in
   * @param defValues the initial Race-based, pre-Guild traits to adjust for Race
  ...
  /**
   * Calculate the base movement speed, based on Action Points (AP = STR+DEX) and height. Movement
   * penalties based on encumbrance (weight carried and weight allowed) are calculated elsewhere.
   * See <code>Person.calcMovement()</code> where movement is adjusted for encumberance.
   * <P>
   * ALGORITHM: For each MVMT_MOD Action Points, the Person can move one block movement (about 1.0
   * feet per second). Movement is constrained between MIN_MVMT (down to 1) and MAX_MVMT (up to 5)
   * for a 10 second interval. There is a +1/-1 height adjustment for extra tall/short Persons after
   * the original base calculation, although no Person's movement can be less than 1 (unless
   * encumbered by too much weight carried). <br>
   * 
   * @param ap action points used for table lookup
  ...
  /**
   * Find how much weight the Person can carry. WeightAllowance depends on the Person's weight. It
   * assums that an average Person (STR=11.5) can carry their own weight without being encumbered;
   * others are pro-rated by STR.
   * 
   * @param str strength of the Person
  ...
  /**
   * Finds the body-type for the individual by calculating which percentile (<i>N-tile</i>) that
   * Person is in. N-tile is calculated by dividing the range by N, where N is the number of
   * categories desired (usually N is the number of descriptions available). The ranges are broken
   * down into N number of subdivisions: div[0] = from MIN_RANGE to div[1], div[1] <= div[2], ...
   * div[N] to MAX_RANGE. <br>
  ...
  /**
   * All Persons start out near the top of their Race's youngest age range. Set the newbie's current
   * age in seconds.
   * 
   * @param startingAge for the Race subclass in years
   ...
   * 
   * @param weight Person's specific weight in inches
   * @param maleWt Average weight of males of the derviced race
   * @param femaleWt Average weight of female of the derived race
   ...
   * 
   * @param height Person's specific height in inches
   * @param maleHt Average height of males of the derviced race
   * @param femaleHt Average height of female of the derived race
   * @return description for the attribute, else null on error
   */
  ...
  /**
   * One-time calculation to get the (randomly-based) height of this race, based on male and female
   * averages. Height has a narrow variance, so HALF_SIGMA is used.
   * 
   * @param maleHt average height of a male of the derived race
   * @param femaleHt average height of a female of the derived race
  ...
  /**
   * One-time calculation to get the (randomly-based) weight of this race, based on male and female
   * averages. Weight has a variance of one standard deveiation, so SIGMA is used.
   * 
   * @param maleWt average weight of a male of the derived race
   * @param femaleWt average weight of a female of the derived race
  ...
  /**
   * Update the default HP Adj by the Person's DEX. There is a Race factor for all Elves
   * 
   * @param defAdj default Adj from new Person
   * @param dex dexterity of the Person, not used for this base method
  ...
  /**
   * Verify that the traits stay within the race limits, else revise them. Traits are STR, INT, WIS,
   * DEX, CON, and CHR
   * 
   * @param traits the actual traits to be verifed for this Person
   * @param minList list of minimum values allowed for traits of derived Race
   * @param maxList list of maximum values allowed for traits of derived Race
   * @return final set of traits, revised if necessary
   * @throws ChronosException if trait list is null or lists are of different sizes
   */
