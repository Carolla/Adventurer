/*
 * Command.java Copyright (c) 2007, Carolla Development, Inc. All Rights Reserved. Permission
...
// This package is needed by the subcommands; see _cmdPackage field
...
/**
 * An abstract base class from which all Commands originate. All abstract services must be
 * defined in the subclass commands. All derived commands are assumed to be in the same package
 * as this <code>Command</code> base class.
 * <p>
 ...
 * <LI>Implement the required constructor(s), <code>init()</code> and <code>exec()</code>
 * methods to override the abstract <code>Command</code> methods.</LI>
 ...
 * constructor.
 * <UL>
 * <LI>A command string <I>name</I> (from the user) that is matched with the command class
 * name.</LI>
 ...
 * Command; others are available through calls that retrieve singleton pointers to the most
 * common objects a new command might need.</LI>
 * <LI>Override the <code>usage()</code> method if the command uses parms on the command
 * line.</LI>
 ...
 * The generic <code>CommandParser</code> command table is searched first, and if not found,
 * then the command table of the current Building is searched. Once found, the
 * <code>CommandFactory</code> is called to create the command object from its command
 * token.</LI>
 * </OL>
 * 
 * @author Alan Cline
...
//  protected final List<String> _parms = null;
   ...
   * polymorphically (for all subcommands).
   * <P>
   * Warning: Be careful that any data stored in the {@code init()} method is still valid by
   * the time the {@code exec()} method is called. Defer collecting needed data as long as
   * possible to avoid mismatching system state.
   * 
   * @return true if the subcommand executed correctly.
   */
  public abstract boolean exec();

  /**
   * This abstract method must be implemented by the subcommand to validate the parms that
   * follow the command word. The non-default parameters to the subcommand are passed as an
   * ArrayList in the order defined in the Format phrase of the subCommand. The arg list
   * contains all words that follow the command name, so any words that must be combined, such
   * as buildings with multi-word names, must be combined in this method.
   * 
...
//    return _name + " " + _parms;
   ...
   * 
   * @return whether command is internal system command or input by user
   */
  ...
  /**
   * Return a generic error message if the Command itself has none. The message is displayed to
   * the User in the user IOPanel
   */
  public String usage()
  {
