...
/**
 * Contains the Person's collection of Items. All weights are in ounces; the
 * client object must convert to lbs if that is the desired unit. Currently, an
 * Item consists of a category, a name, its weight, and a quantity.
 * 
 * @author Alan Cline
...
public class Inventory {
	...
	/**
	 * Some Inventories are meant to be empty, for creating subsets. To populate
	 * the inventory with starting Items, call
	...
	/**
	 * Adds a copy of the incoming Item to the Inventory; used to build
	 * sub-inventories by ItemCategory. If that kind of Item is already in
	 * inventory, then its quantity is increased by the quantity of the incoming
	 * item. Item parm is marked <code>final</code> as a protection from editing
	 * it because it is manipulated independently of the original incoming Item.
	 * 
	 * @param item
	 *            to add (or increase qty count); cannot edit its contents
	...
	/**
	 * Drop (remove) one or more Items of the same kind from Inventory. If there
	 * are multiples, then quantity is decremented. If qty = 1, then Item itself
	 * is removed from the inventory.
	 * 
	 * @param thing
	 *            to drop
	 * @param nbrToDrop
	 *            of those things to drop
	 * @return true if Item(s) are removed successfully, else false if drop
	 *         failed
	 */
	...
	/**
	 * Retrieve an Item reference by name (does not remove it from Inventory but
	 * does allow it to be modified and replaced)
	 * 
	 * @param itemName
	 ...
	 * 
	 * @param category
	 *            which of the available categories should be selected
	 ...
	 * 
	 * @param cat
	 *            category of item to build a subset from
	...
	/**
	 * Count the number of Items (number of unique Items, not quantities) in the
	 * Inventory
	 * 
	 * @return the count
	 */
	...
	/**
	 * Traverse the inventory list, looking for Items by name. This method uses
	 * the equalsIgnoreCase method implemented in the Item class. For now, the
	 * Item must match the full multi-word name of the target, but the search is
	 * case-insensitive. later, "small belt pouch" and "large pouch" should be
	 * found when looking for "pouch".
	 * 
	 * @param target
	 *            the name, or adjective, of the Item being searched
