...
/**
 * Contains a set of skills that a player may assign to his Hero.
 * <code>SkillRegistry2</code> is a read-only singleton and is only initialized
 * once.
 * 
 * @author Alan Cline
 * @version <DL>
...
//    }
    ...
    /**
     * Create the Skill Registry with the tables given, converting each element
     * to a Skill object and saving it in the database.
     */
    @Override
    protected void initialize()
    {
        ...
        try {
            for (int k = 0; k < _racialSkillTable.length; k++) {
                Skill skill = new Skill(_racialSkillTable[k][0],
                        _racialSkillTable[k][1]);
                super.add(skill);           // super is used to highlight inheritance
            }
            System.out.println("\n");
            for (int k = 0; k < _occupSkillTable.length; k++) {
                Skill skill = new Skill(_occupSkillTable[k][0],
                        _occupSkillTable[k][1]);
                super.add(skill);           // super is used to highlight inheritance
            }
