...
/**
 *  Contains all the default information to support the Hero creation: options for hair colors, races,
 *  Occupations and associate Skills, and the Items that make up the default Inventory for a newly 
 *  created Hero. 
 *  <P>    
 ...
 *  <P>
 *  This object is used only when the Registryies need to be created from scratch, and contains all the
 *  hard-coded initialization data. 
 *      
 * @author Alan Cline
 * @version <DL>
 * <DT> Build 1.0		Aug 21, 2012   // original <DD>
 * </DL>
 */
...
//    ArrayList<String> _raceNames = null;
    ...
    /** Skills that are defined for particular races. 
     * These skills must be moved to <code>DungeonWizard</code> later. */
    ...
    /** Starting Items, and weight in ounces, for the Hero's inventory:
     * Total weight for this stash is 426 oz + Peasant starting cash (15.8 gp = 32 oz) = 464 oz. 
     * The String format is used so that the array can be easily iniitialized until in ItemRegistry. 
    ...
    /** Contains same occupations as the Occupation Registry for now. 
     * Put occupations into alphabetical order, which is how the names will be displayed.
     * Each skill must be in the SkillRegistry. See the SkillRegistry for a definition of what 
     * each of the Skills does. */
