...
/**
 * Allows the user to show their Person's many attributes. <BL> <LI>Name: All Persons must have a name by which they are
 * addressed, and are associated with a file by which they are saved.</LI> <LI>Experience Points (XP): Reflects the
 * amount of playing experiences encountered by the Person, and gained toward next Level of ability. XP are awarded for
 * gold and treasure recovered, monsters killed, and puzzles solved. All start at XP = 0.</LI> <LI>Level: Reflects the
 * Person's ability, Level 0 being lowest (usually a Peasant), and Level 1 usually being the lowest Guild member. Levels
 * may rise to Level 10, master of all they command. A Person may rise in Level (be promoted) in their Guild after
 * obtaining enough XP to achieve that Level. All start at Level = 0.</LI> <LI>Hit Points (HP): Reflects the life of the
 * Person. HP are lost each time the Person is damaged in a fight, and are recovered through healing. When the Person's
 * HP = 0, he or she is dead. (There are other ways of dying too, of course.) HP depend on the Person's Klass and Level;
 * Peasants, lowest of all, start at HP = 10.</LI> <LI>Race: Human is the default, but others are available: Dwarf, Elf,
 * Gnome, Half-Elf, Half-Orc, or Hobbit. Each have special penalties and bonuses compared to the Human.</LI> <LI>Klass:
 * Everyone starts as a Peasant, but can become a Fighter, Cleric, Wizard, or Rogue at the proper Guild. All but
 * Fighters inherit special abilities.</LI> <LI>Armor Class (AC): Reflects defensive ability, mostly due to the armor
 * worn. AC also slightly reflects the Dexterity a Person has to avoid being hit.</LI> <LI>Age: Reflects how old the
 * Person is. As the Person plays, and increases in Age, base traits change, and modifiers adjust accordingly. Age also
 * reflects how long the Person spent in play.</LI> <LI>
 * Gender: Females are, on average, shorter, lighter, and have less Strength than Males, but have a higher Constitution
 * and Charisma.</LI> <LI>Height: The height of the Person, affects speed slightly; also defines how high the Person can
 * reach for something. Height depends on the Person's Race.</LI> <LI>
 * Weight: The weight of the Person, affects non-lethal combat somewhat; also limits the strength of bridges the Person
 * may cross safely. Weight depends on the Person's Race.</LI> <LI>Occupation: Although "No Skills" is an option, most
 * occupations reflect some special skill that the Person acquired in earlier life.</LI> <LI>Gold: The amount of wealth
 * the Person owns, and carries about (adding to his load). The <i>Gold</i> value is the number of gold pieces and
 * silver pieces in G.S format (10 sp = 1 gp). Peasants start with the least gold; Guilds will confer a little wealth on
 * the Person, depending on the Guild.</LI> <LI>Gold Banked: The amount of wealth the Person has stashed away, usually
 * in the local Bank. Banks will charge interest on the amount periodically, but you won't have to carry it around with
 * you, and take the risk of being robbed.</LI> <LI>Weight Carried: The weight the Person is currently carrying. Some
 * weight may slow the Person, and there is a limit to how much the Person can carry. Weight carried affects non-lethal
 * combat somewhat; also limits the strength of bridges the Person may cross safely. Weight carried depends on the
 * Person's Strength.</LI> <LI>Max Languages: The total number of languages a Person can learn, in addition to
 * <i>Common</i> and a Race language.</LI> <LI>Languages: All Persons know the <i>Common</i> language, and perhaps a
 * Race language. Each Person has the ability to learn other languages.</LI> <LI>Description: Deduced from hair color,
 * weight, height, gender, race, Klass and CHR, it follows the formula "A [body type] [gender] [race] [klass] with
 * [color] hair. She | He is [CHR value]." Body type is derived from weight, height, body-mass-index ratio, e.g.
 * "tall and thin", or "short and squat". CHR value ranges from "stunningly beautiful" (18) to "horribly scarred" (8).</LI>
 * <LI>Inventory: The list of Items carried by the Person, usually in their backpack. Items are found in the Dungeon, or
 * purchased in the Store. If you don't have it, you can't use it!</LI> <LI>Special Abilities: The behaviors are
 * specific to a Person's occupation, Race, and Klass. Special abilities may involve detecting secret doors (e.g.,
 * Rogues and Elves), throwing spells (e.g. Wizards and Clerics), or just plain luck (e.g, Gamblers).</LI> <LI>Magic Bag
 * (Wizard) or Sacred Satchel (Cleric): If the Person is a spellcaster, then this special container for spell items will
 * appear. Inventory items can be moved to or from the Bag or Satchel to be available for Spells.</LI> </BL>
 * <P>
 * 
 * @see NewHeroDisplay
 * @see HeroDisplayCiv
 ...
 * @author Alan Cline
 * @version <DL>
 *          <DT> Build 1.0 Jan 24 2009 // original <DD> <DT> Build 2.0 Mar 25 2009 // revised to display visible
 *          attributes <DD> <DT> Build 2.1 Apr 15 2009 // allows Saving or canceling after display is shown<DD> <DT>
 *          Build 2.2 May 25 2009 // populate panel with Person's attributes; fixed esthetics<DD> <DT> Build 2.3 Oct 22
 *          2009 // add satiety and fix Save/Open options <DD> <DT> Build 3.0 Nov 18 2009 // made data pane scrollable
 *          and reformatted somewhat<DD> <DT> Build 3.1 Dec 23 2009 // fit panels into pane better and cleaned up
 *          cosmetics <DD> <DT> Build 3.2 May 31 2010 // replaced PDC and DMC with CIV package <DD> <DT> Build 3.0 May 2
 *          2011 // major rewrite for MigLayout manager <DD> <DT> Build 3.1 Oct 6 2011 // minor changes to support MVP
 *          Stack <DD>
 *          </DL>
 */
public class HeroDisplay extends JPanel
{
     ...
     * 
     * @param hdCiv
     *            the intermediary between this GUI and the Person
     */
    public HeroDisplay(HeroDisplayCiv hdCiv)
    {
        _hdCiv = hdCiv;
        // _hdCiv.resetLoadState();
        Mainframe.getInstance().add(this, BorderLayout.WEST);
        setupDisplay();
    }
...
//        frame.changeToLeftPanel(this);
    ...
    public JScrollPane display()
    {
        ...
        mf.add(heroScroll, BorderLayout.WEST);
        mf.repaint();
    ...
    /**
     * The name text field should have the default focus, butt that cannot be
     * done until after the panel is realized and visible. Therefore, the
     * HeroDisplay's caller (MenuBar.NEW Action) must invoke the default
     * component. It calls this method to get which one should be default. This
     * method is created for this because the caller does not know what fields
     * are available in this panel, and it can change with maintenance.
     * 
    ...
    /**
     * Create the person's attributes in a grid panel for display Attributes are
     * unpacked from the data shuttle as they are needed, row by row HEURISTIC
     * LAYOUT: Character attibutes, e.g. XP and Level, go toward the left side;
     * Personal attributes, such as gender, weight, and height, go toward the
     * right side. The more important attributes, e.g. HP and Klass, go toward
     * the top; lesser ones, e.g. description and gold banked, toward the
     * bottom.
     * 
...
//            }
    ...
    /**
     * Builds the inventory "table", showing Category, Items, quantity, and
     * weight (in lbs) and weight (in oz) of each; calls the display civ to pack
     * a data shuttle
     * 
    ...
    /**
     * Format a fixed-field space-filled display string for the Inventory list
     * If values are zero, the String is empty FIELDS: Quantity, Name
     * (left-justified), wt (lb), wt (oz)
     * 
     * @param name
     ...
     * @param lbWt
     *            the weight of the thing in pounds
     * @param ozWt
     *            the fractional pound-weight in ounces
    ...
    /**
     * Creates a lined bordered cell containing a label and its value. The label
     * and its value can be aligned within the component. A JPanel type is
     * returned so that the cell can also get the focus for help listeners.
     * 
     * @param label
     *            the text of the label, can be null
     * @param value
     *            the stringified value to display after the label
    ...
    private JScrollPane makeScrollable()
    {
         ...
         * needed but it set to show the mis-sizing that might occur
         */
    ...
    /**
     * Displays the Person attributes in the shuttle, including inventory and
     * skill set
     * 
     * @param ws
     *            shuttle of PersonKeys containing only String data
     */
    ...
    /**
     * Creates the panel and container fields for the given Person, using a data
     * shuttle from the model to populate the widget.
     * 
     * @param hdCiv
     *            output civ that owns this widget
     * @param ds
     *            shuttle containing String formatted data from the model
     * @param invenShuttle
     *            All Inventory (Item lists) to be displayed are pre-formatted
     *            into Strings, and accessed by the keys in ItemFields
     */
    private boolean setupDisplay()
    {
        ...
        _panelHeight = Mainframe.USERWIN_HEIGHT;
