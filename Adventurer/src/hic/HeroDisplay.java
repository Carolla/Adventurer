...
public class HeroDisplay extends ChronosPanel
{
     ...
     * method. The {@code MigLayout} constructor grid is divided into cells, 12 rows with 6 cells
     * for each row. This is shown by the {@code []0[]} parms. The {@code ins 0} parm means no space
     * is inserted between cells or rows. <br>
    ...
    private JPanel buildAttributePanel()
    {
        ...
        if (klassname.equalsIgnoreCase("WIZARD")) {
            ...
            attribPanel.add(gridCell("Spells in Book: ", _ds.get(PersonKeys.SPELLS_KNOWN)),
                    "span 2, growx, wrap");
            // attribPanel.add(gridCell(" ", ""), "span 1, growx, wrap"); // remainder of line is
            // empty
        } else {
        ...
        attribPanel.add(gridCell("Shield Bash: ", _ds.get(PersonKeys.SHIELD_BASH)),
                "span 2, growx, wrap");
                // attribPanel.add(gridCell(" ", ""), "span 1, growx, wrap"); // remainder of line
                // is empty
...
//                _hdCiv.setActionPanelTitle(getNamePlate());
                ...
                _hdCiv.backToMain(getNamePlate());
            }
        });
        // replace left panel with main buttons panel (returnToMain?)
        // make sure hero is active as title.
...
//            _cancelButton.setEnabled(false);
    ...
    private JPanel buildSkillsPanel()
    {
        ...
        // Section 1: Literacy
        skillPanel.add(gridCell("", _ds.get(PersonKeys.LITERACY)),
                "gaptop 10, span 6, growx, wrap");
        ...
        skillPanel
                .add(buildMultiCell(
                        "SPECIAL " + _ds.get(PersonKeys.RACENAME).toUpperCase() + " SKILLS: ",
    ...
    private void doAlternateSaveAction()
    {
        ...
        switch (choice) {
            ...
            case PROMPT_OVERWRITE:
                ...
                JOptionPane.showMessageDialog(null,
                        _ds.get(PersonKeys.NAME) + CONFIRM_OVERWRITE_MSG,
    ...
    /**
     * Creates a lined bordered cell containing a label and its value. The label and its value can
     * be aligned within the component. A JPanel type is returned so that the cell can also get the
     * focus for help listeners.
     * 
     * @param label the left-aligned text of the label, cannot be null but can be empty
     * @param value the right-aligned stringified value to display after the label, cannot be null
     *        but can be empty
    ...
    // * correctly, the horizontal scrollbar will not be needed but it set to show the mis-sizing
    // that
    // * might occur
    // */
