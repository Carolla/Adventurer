...
/**
 * Allows the author to create a few key attributes of their Hero.
 * A CIV object is called to validate the data and create the Hero, a <code>Person</code> object.
 * <BL> <LI>Name: All Persons must have a name by which they are addressed, and are associated with a file by which they
 * are saved.</LI> <LI>Gender: Females are, on average, shorter, lighter, and have less Strength than Males (the
 * default), but have a higher Constitution and Charisma.</LI> <LI>Hair color: A cosmetic attribute for effect. No
 * semantic information.</LI> <LI>Occupation: Although "None" is an option, most occupations reflect some special skill
 * that the Person acquired in earlier life.</LI> <LI>Race: Human is the default, but others are available: Dwarf, Elf,
 * Gnome, Half-Elf, Half-Orc, or Hobbit. Each have special penalties and bonuses compared to the Human.</LI> <LI>Klass
 * (implied): Everyone starts as a Peasant, but can become a Fighter, Cleric, Wizard, or Rogue when they join a Guild.
 * All but Fighters inherit special abilities.</LI> </BL>
 * 
 * @author Alan Cline
 * @version <DL>
 *          <DT>Build 1.0 Jan 24 2009 // original
 *          <DD>
 *          <DT>Build 2.0 Mar 25 2009 // revised to display visible attributes
 *          <DD>
 *          <DT>Build 2.1 Apr 15 2009 // revised buttons and fields to work with HeroDisplay
 *          <DD>
 *          <DT>Build 2.2 May 25 2009 // a little esthetic cleanup and reflect Person changes
 *          <DD>
 *          <DT>Build 2.3 Oct 15 2009 // set focus on Person name field; add Help actions
 *          <DD>
 *          <DT>Build 3.0 Jun 6 2010 // revised to support NewHeroDisplayCiv
 *          <DD>
 *          <DT>Build 4.0 Apr 2 2011 // major overhaul with non-Swing MigLayout manager
 *          <DD>
 *          </DL>
 */
...
//    private final String WIS = "Wisdom";
    ...
    /**
     * Captures the name text for the hero and the window title;
     * Needed as attribute because data is extracted outside actionListener
     */
    ...
    /**
     * Creates the panel format and places the action components (e.g., radio buttons and
     * drop down boxes). Also creates the associated CIV object to manage the data.
     * The action components are created in private helper methods.
     * 
    ...
    public NewHeroDisplay() throws InstantiationException
    {
        ...
        Border titledBorder = BorderFactory.createTitledBorder(matte,
                        GENERIC_TITLE, TitledBorder.CENTER,
...
//        _backColor = Mainframe.PANEL_COLOR;
     ...
     * panel is realized and visible. Therefore, the NewHeroPanel's caller (MenuBar.NEW Action)
     * must invoke the default component. It calls this method to get which one should be default.
     * This method is created for this because the caller does not know
     * what fields are available in this panel, and it can change with maintenance.
     * 
...
//        mf.setEditFlag(editState);
    ...
    /**
     * Display the various kinds of messages that might happen after submitting a new Hero
     * The type of error, defined in HeroKeys.ErrorType, can be found by checking the suttle's
     * errorType() method. If an exception occurs, the exception message is found in errorMessage();
     * The source of error for invalid data is found in errorSource().
     */
    ...
    /**
     * Updates the ranges for each ability control. Called during instantiation and when race or gender changes.     * 
     */
    ...
    /**
     * Updates the score controls to the correct values. Called after any adjustment to scores is made.
     */
    ...
    /**
     * Indicates when the source component loses focus. In this case, when the
     * Hero name JTextField loses focus so that the data can be extracted. This is necessary
     * only for widgets that cannot have a default.
     */
