...
/**
 * Contains common methods and data for this object's two primary components:
 * <code>XMLTextWriter</code> and <code>PersonContentHandler</code>, the
 * ContentHandler for the validating SAX Parser. It also contain
 * <code>save()</code> and <code>load()</code> methods for serializing Person
 * objects.
 * 
 * @see mylib.dmc.XMLTextWriter
 * @see mylib.dmc.SAXStream
 * 
 * @author Alan Cline
 * @version <DL>
 *          <DT>Build 1.0 Oct 5 2008 // original
 *          <DD>
 *          <DT>Build 1.1 Nov 27 2008 // removed intermediate class and inserted
 *          SAXStream directly
 *          <DD>
 *          <DT>Build 2.0 Feb 22 2009 // updated for Adventurer module and
 *          Person class
 *          <DD>
 *          <DT>Build 2.1 Mar 5 2009 // revised for serialization instead of XML
 *          file
 *          <DD>
 *          </DL>
 */
public class PersonReadWriter {
    ...
    private final String DORMITORY = Chronos.USER_RESOURCES + "dormitory"
    ...
    /**
     * Receive Person PDC object so that this loader has back-reference to its
     * owner.
     */
    public PersonReadWriter() {}

    /**
     * De-serialize a Person object and repopulate it. It will need to have the
     * transient values recalculated before it can be used. It is assumed to be
     * stored in the resource directory with a PERSON_EXT file extension.
     * 
     * @param personName
     *            of the Person object to deserialize
    ...
    public Person load(String personName) {
        ...
        ObjectContainer db = Db4oEmbedded.openFile(
                Db4oEmbedded.newConfiguration(), Chronos.PersonRegPath);
        try {
            // retrieveComplexSODA
     ...
     * 
     * @param fileMap
     *            shuttle of keys designating the data to return
     * @return a data shuttle for displaying directory files for saving the
     *         Person
     */
    public DataShuttle<PersonFileData> packFileData(
            DataShuttle<PersonFileData> fileMap) {
    ...
    /**
     * Serialize the Person to a file, using his/her name. A PERSON_EXT suffix
     * is added when saving to the file system The Klass and Race will be stored
     * with it as components. The Inventory for the Person is saved with the
     * Person.
     * 
     * @param p
     *            Person object to serialize
     * @param pName
     *            filename only; path will be expanded
    ...
    public boolean save(Person p, String pName) {
        ...
        if ((pName == null) || (pName.trim().length() == 0)) {
            MsgCtrl.errMsgln(this,
                    "This Person has no name!. Cannot save him or her.");
            return false;
        }
    ...
    public boolean delete(Person p) {
        ...
        if (p == null) {
            MsgCtrl.errMsgln(this,
                    "There  is no Person to remove. Null received");
            return false;
        }
        ...
        try {
            ...
            db.delete(pToDel);
            MsgCtrl.msgln(this, "Deleted " + p.getName()
                    + " from the Dormitory");
        } finally {
            db.close();
        }
