...
public class NewHeroCiv
{
    ...
    private Map<String, AbilityScoreSelector> _scoreSelectors =
            new HashMap<String, AbilityScoreSelector>();
    ...
    {
        AbilityScoreSelector ss = _scoreSelectors.get(attrib);
        ss.decreaseScore();
    }
    ...
    public AttributeList getAttributeList()
    {
        ...
        for (int i = 0; i < attribArray.length; i++) {
            AbilityScoreSelector ss = _scoreSelectors.get(_attributes.get(i));
    ...
    public void updateScores(String race, String gender)
    {
        ...
        for (String attrib : _attributes) {
            AbilityScoreSelector scoreControl = _scoreSelectors.get(attrib);
            scoreControl.setMinMax();
        }
    }

    /**
     * Transfer the input data from the widget shuttle to the model shuttle. For new Heroes, there
     * are no keys that need to go back to the model; they are provided in the Person constructor.
     * 
    ...
    /**
     * Overrides the base class's do-nothing localAction() to create a new model. The output
     * observer Civ is created to receive the update data from the model later.
     * 
    ...
    private class AbilityScoreSelector
    {
        ...
        /**
         * Set Max and Min values for the control. Called from the constructor and from the UI when
         * race or gender changes.
         */
