...
/**
 * The main civ behind the Mainframe screen. It also contains all the
 ...
 *          <DD>
 *          <DT>Build 1.1 Mar 19 2014 // added current Building for ENTER
 *          command
 *          <DD>
 *          </DL>
 */
public class MainframeCiv {
	...
	public static final String[][] DEFAULT_BUILDINGS = {
			{ "Ugly Ogre Inn", "Bork" },
			{ "Rat's Pack General Store", "Dewey N. Howe" },
			{ "The Bank", "Ogden Moneypenny" }, { "Stadium", "Aragon" },
			{ "Arcaneum", "Pendergast" }, { "Monastery", "Balthazar" },
	...
	private float[][] buildingLayouts = new float[][] {
			{ 0.3f, 0.7f, 0.09f, 0.09f }, // Ugly Ogre Inn
	 ...
	 */
	public MainframeCiv(Mainframe frame, PersonReadWriter personRW,
			AdventureRegistry advReg) {
		_frame = frame;
		_personRW = personRW;
		_advReg = advReg;
		_bdCiv = new BuildingDisplayCiv(_frame,
				(BuildingRegistry) AdvRegistryFactory.getRegistry(RegKey.BLDG));
		// initRegistries();
		createBuildingBoxes();
	...
	private void createBuildingBoxes() {
		for (int i = 0; i < DEFAULT_BUILDINGS.length; i++) {
			...
			BuildingRectangle r = new BuildingRectangle(bName, colorArray[i],
					_frame.getImagePanelSize(), buildingLayouts[i]);
	 ...
	 * 
	 * @param bldName
	 *            the name of the building to open
	 */
