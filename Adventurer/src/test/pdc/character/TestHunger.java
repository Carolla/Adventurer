...
public class TestHunger extends TestCase
{
  ...
  /**
   * Test that the hunger rate reflects the time spent burning calories in activities
   * 
   * @Normal Hunger.calcHunger(long burnTime) ok
  ...
  /**
   * Test that STR points drop as hunger grows, and that up tp 18 pts of STR is reduced to 0 after
   * 72 hours, regardless of Person's maxSatiety. Test for STR recovery also, which will rise in
   * large jumps as the Person eats.
   * 
  ...
  /**
   * Run the hunger rundown tests again, but this time verify the Hunger state with each changes to
   * current satiety and STR.
   * 
  ...
  /**
   * Test the constructor. Verify that all the attributes are correct for the average Person
   * 
  ...
  public void testHunger()
  {
    ...
    Hunger errorHunger = null;
    MsgCtrl.msgln("Error hunger: ");
    try {
      errorHunger = new Hunger(-10.0);
    ...
    Hunger nullHunger = null;
    MsgCtrl.msgln("Null hunger: ");
    try {
      nullHunger = new Hunger(0.0);
    } catch (IllegalArgumentException ex) {
      MsgCtrl.errMsgln("Expected: " + ex.getMessage());
    }
  ...
  /**
   * Take the attributes and compare their values. No return statement because <code>assert</code>s
   * are called directly.
   * 
   * @param hunger the Hunger object to verify
   * @param current current satiety points
   * @param max max satiety points
   * @param burnRate personal burnrate
   * @param lastChange curent satiety at last strength change
   */
  ...
  /**
   * Loop through hunger, showing that STR reduced 18 points in 72 hours 0 hours after satiety=0,
   * STR should be STR - 1; 24 hours after satiety=0, STR should be STR - 6; 68 hours after
   * satiety=0, STR should by STR - 18 72 hours after satiety=0, STR should by STR - 19
   * 
   * @param hunger object to use for this test
   * @param mo hunger object for various Character weights
   * @param hrs number of hours to burn
   * @param group number of hours between verifications
   */
