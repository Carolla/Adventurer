/**
 * A01_N001a. Integration test for A01_CreateHero Copyright (c) 2010, Carolla Development, Inc. All
 * Rights Reserved
 * 
...
/**
 * Integration Test to create a person and verify that they were created properly
 * 
 ...
 *          </DL>
 */
public class A01_N001a extends TestCase
{
  ...
  private final String IP_NAME = "Potato";
  ...
  private final String IP_OCCUPATION = "Farmer";
  ...
  private final int OP_HEIGHT = 81;
  private final int OP_WEIGHT = 175;
  ...
  private final String OP_DESCRIPTION =
      "A giant male with no hair. He is weather-beaten and tough.";

  // Skill Tab
  private final String[][] OP_SKILLS = {{"Predict Weather",
  ...
  /**
   * Create the objects needed for testing; called before each test. The methods are called, as
   * independent of the Race and Klass as possible.
   */
  ...
  public void testA01_N001()
  {
    ...
    for (int j = 0; j < OP_SKILLS.length; j++) {
      int k = 0;
      {
        ...
        assertTrue(skills.get(j).getDescription().equalsIgnoreCase(p_skill.getDescription()));
      }
    }
   ...
   */
  /**
   * Pack the internal fields then send the new Hero data to the Civ to be validated. Scavanged
   * method from NewHeroDisplay
   * 
   * @param ds Data shuttle with default data from NHCiv
