 ...
 * must convert to lbs if that is the desired unit. 
 * Currently, an Item consists of a category, a name, its weight, and a quantity.
 * 
 * @author Alan Cline
 * @version <DL>
 * <DT> Build 1.0		Mar 18, 2009	// original <DD>
 * <DT> Build 2.0		Jun 7, 2009  	// updated for Item objects instead of Strings <DD>
 * <DT> Build 2.1		Oct 21, 2009   // changed weights to be oz ints instead of codes <DD>
 * <DT> Build 2.2     Apr 11 2011    // TAA uncommented money for testing <DT>
 * <DT> Build 2.3     Jun 13 2011    // ABC added data shuttle handling <DT>
 * <DT> Build 2.4     25 Nov 2011    // ABC replaced double cash with Items Gold and Silver <DT>
 * </DL>
 */
public class Inventory implements Serializable
{
    ...
    /** The amount of money the Person has saved up; initially, it is none. Units represent
     * gold, decimal fraction represents silver. */
    private double _goldBanked = 0.0;

    /** Armor class if Person has no armor. On a scale of 1-20 (20 being highest), 
     * no armor = 10 implies a 50% chance to be hit and damaged. */
    static public final int NO_ARMOR = 10;
    /** Armor class depending on Dex and armor */
    private int _ac = NO_ARMOR;

    /** Inventory is currently implemented as an ArrayList<Item> (EnumMaps 
     * cannot have duplicate keys). Each element is an Item object: category, 
     * name, weight (oz), and quantity.  */
    private ArrayList<Item> _inventory = null;

    /** Starting Items, and weight in ounces, for the Person's inventory:
    * Total weight for this stash is 426 oz + Peasant starting cash (15.8 gp = 32 oz) = 464 oz. 
    * The String format is used so that the triple-array can be easily iniitialized until in ItemRegistry. 
    * Each Item := Category, Name, Weight (ea), and Quantity.
    * Cash will be set from the Klass initCash() method as Gold and Silver, both of Category CASH 
    */
    ...
    /** Some Inventories are meant to be empty, for creating subsets.
     * To populate the inventory with starting Items, call 
     * <code>initStartingInventory()</code>.
     */
    public Inventory()
    {
     ...
     * ItemCategory. If that kind of Item is already in inventory, then its quantity is increased 
     * by the quantity of the incoming item. 
     * Item parm is marked <code>final</code> as a protection 
     * from editing it because it is manipulated independently of the original incoming Item.
     *  
    ...
    /** Set the armor class, on a scale of 10 (no armor) to full plate mail and shield (18).
     * Some low DEX values or cursed items may bring AC less than No_Armor but it
     * can never be less than 1. Magic armor allows the AC to be higher than 18. 
     * For now, until Inventory and Items are implemented fully, the Person starts with
     * no armor.
     *  
    ...
    /** Calculate the real-time value for:
     * Overbearing = AP + (1 per 25 lb weight carried); +4 if wearing metal armor
     * Grappling = AP + DamageMod + ToHitMissileMod; -4 if wearing metal gauntlets
     * Pummeling = AP; +4 if wearing metal gauntlets
     * Shield Bash = AP if the Hero has a shield; else 0
    ...
    /** Drop (remove) one or more Items of the same kind from Inventory.  
     * If there are multiples, then quantity is decremented. 
     * If qty = 1, then Item itself is removed from the inventory. 
     * 
     * @param thing to drop
    ...
    //	/** Cash is stored as two items: gold and silver, and kept separate to maintain the weight load.
    ...
    /** Return the Item for the given index. This is a convenience method for traversing
     * the inventory easily from within.
     *   
    ...
    /** Retrieve an Item reference by name (does not remove it from Inventory but does 
     * allow it to be modified and replaced) 
     *  
     * @param itemName  the name, or adjective, of the Item being searched; it should
    ...
    /** Confirm that the String of inventory items are properly loaded
     * Later, the Inventory will be read from the <code>DungeonWizard</code> file
     * 
    ...
    */
    ...
    /** Traverse the inventory list, looking for Items by name. This method uses the
    * equalsIgnoreCase method implemented in the Item class. For now, the Item
    * must match the full multi-word name of the target, but the search is case-insensitive.
    * later, "small belt pouch" and "large pouch" should be found when looking for "pouch".
    *   
    ...
    */
    ...
    /** Assign the starting set of Items to the Person's inventory: clothing, cash, provisions, 
     * and other minimums for adventuring. 
     * Each key is an Item containing its category, name, weight (oz), and quantity. 
     * 
    ...
    public class MockInventory
    {
        ...
        /** Test that the starting inventory, when a different string list is used, still works.
         * Replace the default list with the given new list
         * 
