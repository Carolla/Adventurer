...
/**
 * TODO: [3] Revise the speed calc to have some affect at 1.5 weight allowance,
 * instead of only between 1 and 2 times weight allowed.
 */

/**
 * Generates a new Hero for playing <i>Adventurer</i>, according to the rules
 * documented in the <i>Developers<kbd>'</kbd> Reference Guide</i> of
 * <i>Adventurer: A Fantasy Role-Playing Adventure Program for the
 * <b>Chronos</b> Gaming Framework</i>, Al Cline, February 12, 2002, Revised May
 * 5, 2012. See section 3, pp12-24.
 * <P>
 * <OL>
 * <LI>PROFILE DATA: Collect the input data to create the <code>Person</code>
 * object: name, gender, occupation, hair color, and name of Race. All Heroes
 * are created by default as Peasant Klass.</LI>
 * <LI>PRIME TRAITS: Each Person has six prime traits on which all actions and
 * reactions are based: Strength (STR), Intelligence (INT), Wisdom (WIS),
 * Dexterity (DEX), Constitution (CON), and Charisma (CHR). Peasants are
 * initialized to fixed mediocre values except for CON and CHR. The Hero's
 * traits for the others are generated based on the <code>Guild</code> of which
 * Klass they join later. All traits are rolled using the <i>4d6 - lowest</i>
 * algorithm</LI>
 * <LI>ADJUST FOR RACE: Various non-human races have bonuses and penalties;
 * human Heroes are the norm with no adjustments.</LI>
 * <LI>ADJUST FOR GENDER: Females are weaker than males but are more charismatic
 * and have a higher constitution, so they get CON+1, STR-1, CHR+1 adjustments.</LI>
 * <LI>SET INITIAL AGE: All Heroes start as Young Adult, whose actual age
 * depends on the Hero's Race. Adjustment for Young Adult age: WIS-1, CON+1.</LI>
 * <LI>VERIFY RACE LIMITS: After adjustments, ensure that no trait is greater
 * than possible for that particular Race. Humans range from 8-18 (except STR,
 * which ranges from 8-19). If the trait is outside the limit, it is set to the
 * min or max limit.</LI>
 * <LI>HEIGHT and WEIGHT: Init <i>height</i> and <i>weight</i> of the Person.
 * These values are random, but constrained to a normal (Gaussian) demographic
 * for the Race and gender about a Racial mean value.</LI>
 * <LI>DESCRIPTION: Based on the Person's height, weight, hair color, and
 * charisma, generate a text description about that Person's appearance. High
 * charisma results in positive connotations and adjectives, and low charisma
 * gives a similar description but with negative connotations.</LI>
 * <LI>STR Modifiers: Init <i>ToHitMeleeAdj</i>, <i>DamageAdj</i>, and
 * <i>WeightAllowance</i><br>
 * (a) A stronger Person is more likely to break through his/her opponent's
 * defenses and score a hit with a hand-held weapon (<i>ToHitMeleeAdj</i>), <br>
 * (b) and do more damage with that hit (<i>DamageAdj</i>). <br>
 * (c) The amount of weight the Person can carry (<i>WeightAllowance</i>)
 * depends on the Person's STR. It is assumed that a Hero of average STR can
 * carry their own weight without being encumbered (losing movement speed).
 * Stronger and weaker Persons can carry more or less weight, respectively.</LI>
 * <LI>INT Modifiers: Intelligence drives languages known and learnable, and the
 * Literacy skill of the Hero. <br>
 * (a) Set the languages known and possible for the Person (all Heroes know
 * <i>Common</i>), determine any possible Race-based language and count the
 * total <i>langsKnown</i> (one or two at this point). <br>
 * (b) Calculate the additional INT-based <i>maxLangs</i> that the Person can
 * learn. <br>
 * (c) Find the <i>Literacy</i> ability: Illiterate, Read, or Read and Write.</LI>
 * <LI>WIS Modifier: Init the <i>MagicAttackAdj</i>, a Person's ability to
 * defend against magical attacks. Dwarves, Gnomes, and Hobbits get extra
 * bonuses for their natural resistence to magic.</LI>
 * <LI>CON Modifiers: Init <i>HitPointMod</i>, the bonus/penalty given for
 * high/low physical stamina when determining Hit Points. Also used for Guild
 * promotions.</LI>
 * <LI>DEX Modifiers: Init <i>ToHitMissileMod</i> and <i>ArmorClassMod</i>, the
 * bonus/penalty to aim missile weapons, as in better/worse eye-hand
 * coordination. Somes Races, like Elves, have a natural ability and get an
 * bonus to this Mod. The Hero<kbd>'</kbd>s defensive ability due to ability to
 * dodge and duck affects their AC, which is adjusted for a high/low DEX because
 * they are better or worse at avoiding being hit.</LI>
 * <LI>HUNGER: Calculate the Hero<kbd>'</kbd>s burn rate before getting hungry,
 * weak, or even starving to death. All Heroes normally burn 15 Satiety Points
 * (SP) per pound weight, and all start with full bellies.</LI>
 * <LI>EXPERIENCE: The Hero's power is measured through a large-grained metric,
 * <i>Level</i>, and a fine-grained metric, <i>Experience Points (XP)</i>. For
 * every few thousand XP gained, the Hero can rise in Level and power. <br>
 * (a) Level: All Heroes start at 0 level. Peasants need 200 XP to reach Level 1
 * and be eligible to join a Guild. This requirement forces the Hero to explore
 * and gain some starting experience.<br>
 * (b) Experience Points (XP): All Heroes start at 0 XP. XP are earned by
 * defeating monsters, solving puzzles, and obtaining gold and valueables.</LI>
 * <LI>HIT POINTS: Set the initial <i>Hit Points (HP)</i> to 10, plus whatever
 * HP Mod the Hero has. Peasants start with a fixed bare minimum; later, Guilds
 * will bestow additional random number of Klass-specific HPs to the Hero.</LI>
 * <LI>INVENTORY: Create a new <code>Inventory</code> object and initialize it
 * with the default Items for new Hero. All Peasants start with the same bare
 * minimum. Some Occupations have a Kit, which should also be added to the
 * Hero<kbd>'</kbd>s Inventory.</LI>
 * <LI>CASH: Set the starting <i>gold</i> for the Peasant, measured in gold
 * pieces (gp) and silver pieces (sp), where 1 gp = 10 sp. All Peasants start
 * with the same bare minimum, which is less than the poorest of the Guild
 * classes, the Magic-User. The cash's weight is added to the Hero's load. Set
 * the <i>gold banked</i> (saved) to 0/0.</LI>
 * <LI>ACTION POINTS (AP): Add STR and DEX to yield the AP. The AP modifiers
 * (<i>overbearing, grappling, pummeling, shield bash, movement</i>) are based
 * on quickly changing conditions, so are calculated as needed. The AP value is
 * frequently needed and somewhat stable so is kept as an attribute.<br>
 ...
 * <LI>ARMOR CLASS (AC): Set <i>armor class</i> to 10 + AC Mod. Hero has no
 * armor yet.</LI>
 * <LI>SPEED. <i>Speed</i>, or <i>base movement</i>, depends on the AP and
 * weight carried by the Hero. Calculate the base movement, which can change as
 * the Hero plays.</LI>
 * <LI>SPECIAL ABILITIES: Assign any special abilities that comes with the
 * <i>occupation</i> attribute, then add the Race-based <i>special
 * abilities</i>. Later, after the Hero joins a Klass-based Guild, special Klass
 * abilities are added. Human Peasants have none.</LI>
 * </OL>
 * <P>
 * After the Hero is generated, display the Hero and certain traits to the user.
 * The user sees only attributes that they would directly experience, such as
 * height and weight. The traits, e.g., STR and CON, are not shown. CHR can be
 * inferred by the Hero's description.
 * <P>
 * IMPLEMENTATION NOTE: The <code>Person</code> class includes a specific
 * <code>Race</code> subclass and a specific <code>Klass</code> subclass as
 * component objects. It also is assocated with an <code>Inventory</code>
 * object. All calls to the component objects go through a Person wrapper. <br>
 * Attributes that are set in the constructor and change rarely are defined with
 * <code>init___()</code> methods. <code>init</code> methods are not called
 * outside the constructor, and are only called once. <br>
 * Attributes frequently used and with low volatility, or are part of the class
 * interface, are calculated and saved as attributes for efficiency with methods
 * that are often prefixed as <code>set___()</code> methods. <br>
 * Values that change frequently, or depend on values that do, like weight of
 * Items carried, or Armor Class, are calculated when needed, and not defined as
 * attributes. These methods are prefixed as <code>calc___()</code> methods.
 * 
 * @author Alan Cline
 * @version <DL>
 *          <DT> Build 1.0 Feb 14 2009 // original <DD> <DT> Build 1.1 Feb 28
 *          2009 // modified after Race and Klass re-design <DD> <DT> Build 1.2
 *          Mar 4 2009 // modified for serialization <DD> <DT> Build 1.3 Mar 14
 *          2009 // modified for factor methods and cleaner analysis <DD> <DT>
 *          Build 1.4 Apr 24 2009 // remove AP mods and movement as attributes
 *          because they will be calculated as needed <DD> <DT> Build 2.0 Apr 26
 *          2009 // added MockPerson inner class, and consequently was able to
 *          make public and private methods as they should be. Also added the
 *          <code>init</code> methods for fundamental attributes. <DD> <DT>
 *          Build 2.1 May 10 2009 // added age attribute and cleaned up <DD> 
 *          <DT> Build 2.2 Jan 2 2010 // replaced occupation string with
 *          Occupation object <DD> <DT> Build 3.0 Jul 3 2010 // support for CIV
 *          and packFields() <DD> <DT> Build 3.1 Mar 26 2011 // revise for
 *          refactoring requirements spec <DD> <DT> Build 3.2 Apr 10 2011 // TAA
 *          uncommented methods, ensured function for testing <DD> <DT> Build
 *          3.3 Oct 2 2011 // refactored for MVP Stack implementation <DD> <DT>
 *          Build 3.4 Nov 3 2011 // polished for final integration testing <DD>
 *          </DL>
 */
public class Person implements Serializable, IRegistryElement
{
    ...
    /**
     * Minimum intelligence to be able to learn any new language except Common
     * and Race lang
     */
    static private final int MIN_LANG_INT = 10;

    // // TODO: Change the traits to an enum for convenience, and use an
    // ArrayList
    // // instead of an int[] */
    // /** Number of Personal traits */
    // static public final int NBR_TRAITS = 6;
    // /**
    ...
    // and
    // * gives advantage for quiet attacks in battle; DEX is the prime requisite
    // * for a Rogue.
    // */
    // static public final int DEX = 1;
    // /**
    // * Intelligence: Ability to identify items, magical or otherwise, learn
    // and
    // * cast arcane spells; INT is the prime requsite for a Wizard.
    // */
    // static public final int INT = 2;
    // /**
    // * Wisdom: Application of knowledge, ability to cast clerical spells,
    // works
    // * against undead creatures and to protect against magical attacks; WIS is
    // * the prime requisite for a Cleric.
    // */
    // static public final int WIS = 3;
    // /**
    ...
    /**
     * Collection of Items that Person carries when first starting out. This
     * object affects the Person's Action Points, and contains the Person's
     * money.
     */
    ...
    /**
     * Array of key traits of the Person: STR, INT, WIS, CON, DEX, CHR.
     * Generated for Klass and adjusted for Race, but shared commonly with all
     * elements of Person.
     */
    protected AttributeList _trait = null;

    /**
     * The list of languages the Person knows; default to 2 at init (updated as
     * player plays). Human Peasants start only with Common. Other races have
     * also a racial language; mixed races, e.g. half-elf, have a chance at a
     * racial language. Rogues also know "Thieves' Cant", a Klass-based
     * language.
     */
    private ArrayList<String> _langs = new ArrayList<String>();
    /**
     * The special abilities for the Person, depending on the occupation, Race
     * or Klass. (if none, then assign "None")
     */
    private ArrayList<Skill> _skills = new ArrayList<Skill>();

    // Derived from STR attribute; low volatility
    /**
     * Adjustment to chance To Hit (cause damage) with melee weapons due to
     * extra STR
     */
    ...
    /**
     * The weight (ounces) a Person can carry before being encumbered. Average
     * Person can carry their own weight.
     */
    ...
    /**
     * Action Points (STR + DEX) determine non-weapon battle outcomes; low
     * voltatilty but here for efficiency
     */
     ...
     * 
     * @param name
     *            of the Person, default filename under which it is saved
     * @param gender
     *            male or female Person
     * @param occup
     *            selected occupation of the non-klass Person
     * @param hairColor
     *            selected hair color for this Person
     * @param raceName
     *            the concrete subclass object of Race, e.g., Human or Hobbit;
     * @param klassName
     *            the concrete subclass object of Klass, initially a Peasant;
     * @param attribs
     *            the attribute scores of the Person.           
     * 
     * @throws InstantiationException
     *             if any of the input parms are null
     */
    public Person(String name, String gender, String occup, String hairColor,
        ...
        /*
         * CONSTRUCT THE PERSON OBJECT General heuristic: use set methods for
         * initiating low volatility or unchanging attributes; use calc methods
         * if they change more frequently.
         */
    ...
    public Person(String name, String gender, String occup, String hairColor,
    	...
    	this(name, gender, occup, hairColor, raceName, klassName, Klass.createKlass(klassName).rollInitialTraits());
    	
    }

    /*
     * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     * PRIVATE METHODS TO SUPPORT THE CONSTRUCTOR
     * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     */
    ...
    /**
     * Check if the Person is encumbered by carrying more weight than allowed.
     * If he is encumbered, his movement is slowed by the amount of over-weight
     * carried until if too much weight, he will not be able to move at all. If
     * his load is less than the weight allowed, he will not be impeded.
     * Encumberance applies for loads over the weight allowed to twice the
     * weight allowed. The encumberance factor is multiplied times the speed of
     * the person to get the net speed. <br>
     * ALGORITHM: The encumberance E is a factor going from 1 to 0 as the ratio
     * R (= load / wt allowed) goes from 0 to 2 (or greater). If R <= 1, E = 1;
     * if R >= 2, E = 0 (no movement). The general equation for this linear
     * calculation is <br>
     * E = -R + 2 for (1 <= x <= 2). <br>
    ...
    /**
     * Get a linear adjustment mod from a low, middle, or high bracket for a
     * particular trait value. If a trait is below a lower limit, the mod is
     * offset negative; else if the trait is above a higher limit, the mod is
     * offset positive; the middle bracket gives 0 mod value. <br>
     * This algorithm is used frequently by both Race and Klass objects, so it
     * is generalized into a consistent lookup, and placed in this class.
     * 
     * @param targetValue
     *            to search in table
     * @param lowValue
    ...
    /**
     * Display the Hero data. Create the civ and widget, then push each of three
     * data shuttles to be converted and displayed: Person's attributes,
    ...
    /*
     * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ COMMON
     * GETTERS and SETTERS for BASE CLASS and COMPONENTS
     * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     */

    /**
     * Slight DEX adjustment to avoid damage by adjusting the Person's armor
     * class.
     * 
    ...
    /**
     * What the Person sees if looking in a mirror at mid-range, or what someone
     * else will see when looking at this Person.
     * 
     ...
     * 
     * @param pfd
     *            person file data needed for saving the person
    ...
    /**
     * The life force (Hit Points) of the Person. As Person is dead when HP go
     * to zero.
     * 
    ...
    /**
     * Return the total weight (in ounces) of all inventory items, without gold
     * weight
     * 
    ...
    public boolean equals(Object obj)
    {
        ...
        if (_Race == null) {
            if (other._Race != null)
                return false;
        } else if (!_Race.getRaceName().equals(other._Race.getRaceName()))
            return false;
        if (_gender == null) {
    ...
    /**
     * Concatenate all the languages known by the Person into a String list for
     * display
     * 
    ...
    /**
     * Return the experience level of this Person. All Peasant's start at Level
     * 0, then get promoted by a Guild to higher levels after so many Experience
     * Points achived.
     * 
    ...
    /**
     * Return the literacy of the Person: Illiteration, Reading, Writing (which
     * assumes reading).
     * 
    ...
    /**
     * Some WIS Persons and certain races are resistance to magical attacks.
     * Clerics can use magical attack adjustment to increase certain kinds of
     * attacks.
     * 
    ...
    /**
     * Based on INT, Persons have a limit on how many languages they can learn
     * (besides Common and their possible race language.
     * 
    ...
    /**
     * Person's with higher DEX are more likely to hit with a missile weapon,
     * e.g. bow or thrown dagger. Elves are also extemely skilled with the bow,
     * and have a slightly higher adjustment.
     * 
    ...
    /**
     * How much the Person can carry before being encumbered (being slowed down)
     * depends on STR
     * 
    ...
    /**
     * The value that characterizes experience playing. As XP are achieved, the
     * Person can be promoted to a higher level for more power.
     * 
     ...
     * 
     * @param target
     *            the name, or adjective, of the Item being searched; it should
     *            find "small" equally as well as "shield"
    ...
    /**
     * Determine if the Person has a particular skill by traversing through the
     * skill list. Skills are built from the occupation, INT, race, and klass
     * abilties
     * 
     * @param target
     *            the name of the skill being searched
    ...
    /*
     * Load the Person's displayable attributes, inventory, and skills into the
     * data shuttle
     * 
     * @param ds shuttle to receive the Person data, as requested by the keys
     * within it
     ...
     * 
     * @param filename
     *            the filename for the Person file, without the extension. The
     *            chosen file originates from the HIC to restore a particular
     *            Person, and is passed through the Person to its component
     *            <code>PersonReadWriter</code>.
     ...
     * 
     * @param pName
     *            the name of the person to be summon, this person is passed
     *            through the Person to its component
     *            <code>PersonReadWriter</code>.
     * @return false if a problem occured with the loading
     * @see PersonReadWriter
     */
    ...
    /**
     * Serialize the Person to a file. All components will be stored with it as
     * components. The file is saved in the default resource directory.
     * 
     * @param filename
     *            the filename for the Person file, without the extension. The
     *            chosen file originates from the HIC to restore a particular
    ...
    /*
     * Unloads the already-validated data from the shuttle, indicated by the
     * key, and casts it into this object's attributes
     * 
     * @param ds contains the data to load into this object
     * 
     * @see mylib.pdc.ObservableModel#unload(mylib.civ.DataShuttle)
     * 
     * @throws ClassCastException if the value cannot be cast properly during
     * validation
     */
     ...
     */
    /**
     * Calculate the speed of movement for the Person, which depends on Action
     * Points and weight carried. The Racial base movement is adjusted by the
     * Inventory weight carried and pro-rated against the Person's weight
     * allowance.
     * 
     * // * @param ap Action Points // * @param wtAllow the weight that can be
     * carried before encumberance
     * 
     * @param load
     *            the load being carried
     * @return the Person's speed per round, (2 to 5 generally if not
     *         encumbered)
     */
    ...
    /**
     * Check if the first letter of a string is a vowel so the proper article (A
     * or An) can be placed in front of it.
     * 
     * @param target
     *            the string to check if the first letter is a vowel or not
    ...
    /**
     * Build the physical appearance of the Person, without regard to what they
     * are wearing or anything that can drastically change. The description
     * depends on height, weight, race, klass, hair color, gender and charisma.
     * A special racial note is appended to all races except Human. <br>
     * NOTE: Although the description is actually a user-interface attribute, it
     * is generated and saved in the Person object for expedience to avoid the
     * high overhead of calculating it.
     * <P>
     * Template for the attributes in description: <br>
     * "A [height_descriptor] and [weight_descriptor]" +
     * "[gender] with [color] hair" + "and [racial note]". [She | He] is [CHR
     * reflection]". <br>
     * 
    ...
    /**
     * Assign the languages natural to the Person. All Persons know the Common
     * language, and non-human races might also have a race language. Klass
     * languages are assigned during Guild activity, and not during (Peasant)
     * initialization
     * 
     * @return an arrayList containing the language(s) intialized for this
     *         Person
     */
    ...
    /**
     * Add the literacy skill based on intelligence. Anyone who can write, can
     * also read.
     * 
     * @param intelligence
     *            of the person, determines literacy skill
     * @return read-only, read/write, or illiterate; or null if there was a
     *         problem
     */
    ...
    /**
     * Calculate the number of learnable languages for the Person, depending on
     * his INT. NOTE: Learning a language means that the Hero can only speak and
     * understand it. He or she must have the proper Intelligence to be Literate
     * and be able to read and/or write it.
     * 
    ...
    /**
     * Call the initial skill set assignments. Any existing skill list is
     * cleared.
     * 
    ...
    public class MockPerson
    {
        ...
        /**
         * Person's with higher STR are more likely to hit with a melee
         * (non-missile) weapon.
         * 
        ...
        /**
         * Person's with higher STR can get damage easier when they hit with a
         * melee (non-missile) weapon.
         * 
         * @return the adjustment to hit an opponent with a melee weapon
         */
         ...
         * always known, so maxLangs is a minimum of 1.
         * 
         * @param intelligence
         *            of Person
