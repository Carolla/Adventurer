 ...
 * points to represent calories. A person's initial maximum satiety points are
 * based on the nutritional rule of 15 calories per pound times their weight.
 * This class forces persons to risk eating things in the dungeon, such as food
 * found, potions and poisons. As they burn calories, they will move from a FULL
 * state to a STARVING state. If the person has no satiety points, he will begin
 * losing STR points until dead (or until he eats something.)
 * <p>
 * For example, for an average person, max satiety points = 150 lb * 15 sp/lb *
 * 3 days = 6750 sp (which is 2250 per day, or 0.026 sp/sec for calculating
 * calories burned during time-based activities).
 * <p>
 * Different foods have different sp's, and each edible item will have a satiety
 * amount. Standard rations contain 2000 sp, enough for about a day (24 hours)
 * of activity. Water has 0 sp, an apple = 20 sp, roast beef with goat's milk =
 * 3000 sp. <br>
 ...
 * @author Alan Cline
 * @version <DL>
 *          <DT> Build 1.0 Oct 30, 2010 // original <DD> <DT> Build 1.1 April 10
 *          2011 // TAA added check for invalid weight <DD>
 *          </DL>
 */
...
public class Hunger implements Serializable {
	...
	/**
	 * The satiety points burned per second for the Person after activity; a
	 * convenience calculation
	 */
	private double _personalBurnRate = Constants.UNASSIGNED;
	/**
	 * STR interval set so that 18 STR points are lost in 72 hours, which is
	 * also the amount of time curSatiety goes from maxSatiety goes to 0
	 */
	private double _strDropInterval = Constants.UNASSIGNED;
	/**
	 * The Person-specific satiety increment by which STR is lost during
	 * starvation
	 */
	...
	/**
	 * Amount of satiety points per pound required for a Hero's weight: 15
	 * calories per lb
	 */
	...
	/**
	 * The max STR value for purposes of Hunger calculations, starving, and
	 * recovery
	 */
	...
	/**
	 * Sets the maximum satiety (FULL) at the start, using the person's weight:
	 * each person consumes his energy at 15 cal/lb, so a person has 3 days
	 * worth of calories as a maximum. The current satiety points are calculated
	 * with each activity the Person does. The person starts losing STR after
	 * all satiety points are exhausted.
	 * 
	 * @param weight
	 *            the number of lbs the person weights (excluding inventory)
	 * @return satiety of Person in satiety points
	 */
	public Hunger(double weight) {
		if (weight > 0) {
	 ...
	 * The current and max satiety values are calculated, along with a decrement
	 * to indicate a particular hunger state. The hunger state is used to
	 * determine if the Person is hungry enough to start losing STR points. <br>
	 * Later, commands will be modified to allow extra higher (e.g., fighting)
	 * or lower (e.g., resting) burn rates.
	 * 
	 * @param burnTime
	 *            the time the burnrate is applied; typically, the time a
	 *            user-command takes to execute
	...
	/**
	 * Calculate any strength penalties or recovery due to changes in hunger
	 * over time. Compare the drop in satieties at the time Strength change with
	 * the satiety now. Algorithm: 18 STR points are lost in 72 hours, which is
	 * the StrDropInterval based on the maxSatiety of the person.
	 * 
	 * After 72 hours, STR is 18/72 pts/hr = 0.25 pts per hour. Person will be
	 * WEAK for 0 - 11 hours (incusive), resulting in STR reduction of -4.
	 * Person will be FAINT for 11- 48 hours, resulting in STR reduction of -5
	 * to -13. Person will be STARVED after any time after 48 hours (STR-12)
	...
	/**
	 * Algorithm: If the Person's current satiety points are within a certain
	 * range, Person will traverse through the following stages. <br>
	 * When current satiety reaches 0, Person loses 1 STR point. As satiety
	 * decreases, Person loses STR at a rate such that after 72 hours, STR -=
	 * -19. All STR dependent attributes are recalculated, notably AP, speed,
	 * and weight allowance each time Person loses STR. At STR = 3, Person will
	 * become unconscious; speed probably has gone to 0 before that. </LI>
	 * 
	 * To find these states of hunger, the maxSatiety is broken into brackets of
	 * percent. In the list below, values in square brackets are inclusive;
	 * values in parenthesis are. <BL> <LI>FULL: Current satiety [90, 100]%.
	 * Person cannot eat when FULL, and cannot eat more than 100% of their
	 * satiety points.</LI> <LI>NOT_HUNGRY: [30, 90) %</LI> <LI>HUNGRY: (30, 0]
	 * %.</LI> <LI>WEAK: [-40, 0] %</LI> <LI>FAINT: [-100, -40)</LI> <LI>
	 * STARVED: < -100 Hero is dead</LI> </BL>
	 * 
	...
	public class MockHunger {
		 ...
		 * 
		 * @param newSat
		 *            the value to set the current satiety points to
		 */
		 ...
		 * 
		 * @param newStrAdj
		 *            the value to set the current STR adjustment for Hunger
		 */
