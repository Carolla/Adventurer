 ...
 * excellent Fighters because of their naturally high dexterity, and excellent
 * magic-users because of their high magical abilities. Elves have the following
 * differences from Humans: <BL> <LI >Tend to be shorter and smaller than Humans
 * </LI> <LI >Tend to slightly higher Constitution and Dexterity</LI> <LI >
 * Racial language in addition to Common: Elvish</LI> <LI >High resistance to
 * magic</LI> <LI >Special abilities: Infravision = 60', and</LI> <LI><i>
 * Tingling</i>: Detecting hidden or secret openings if close enough, whether
 * trying or not</LI> <LI><i> Archery</i>: +1 To Hit with missle weapons</LI>
 * <LI><i> Move Silently</i>: Same as first level Rogue of the same DEX</LI>
 * </BL>
 * 
 * @author Alan Cline
 * @version <DL>
 *          <DT>Build 1.0 1 April 2012 // original
 *          <DD>
 *          </DL>
 */
@SuppressWarnings("serial")
public class Elf extends Race implements Serializable {
    /** Average height (inches) for a Elf male */
    private final int AVG_HEIGHT_MALE = 60;
    /** Average height (inches) for a Elf female */
    private final int AVG_HEIGHT_FEMALE = 54;

    /** Average weight (pounds) for a Elf male */
    private final int AVG_WEIGHT_MALE = 100;
    /** Average weight (pounds) for a Elf female */
    private final int AVG_WEIGHT_FEMALE = 80;

    /** The minimum Trait values for a Elf: STR, INT, WIS, DEX, CON, CHR */
    private final int[] _minTraits = { 8, 8, 8, 9, 7, 8 };
    /** The maximum Trait values for a Elf: STR, INT, WIS, DEX, CON, CHR */
    private final int[] _maxTraits = { 18, 18, 18, 19, 17, 18};

    /**
     * The oldest ages in each of the Elf age brackets (see DMG, p13; use Drow
     * elf table)
     */
    private final int[] _elfAgeBracket = { 100, 400, 600, 800, 1000 };
    ...
    private final String RACE_DESCRIPTOR = "pointed ears";

    /** The names of the special abilities that come with being a dwarf */
    private final String[] _elfSkills = { "Infravision", "Resistance to Sleep",
    ...
    // private final int SLEEP_RESIST = 90;
    // private final int CHARM_RESIST = 90;
    // private final int TINGLING_ACTIVE = 66;
    ...
    /**
     * Elfs have a few special Racial skills. The names are looked up, and the
    ...
    // TODO: Move to CIV
    // /** Retrive the specific category for the Elf's age. See Race class for
    // list.
    // *
    ...
    // {
    // String cat = null;
    // // Find what age the Elf category falls into
    // try {
    // cat = Age.calcAgeCategory(_currentAge, _elfAgeBracket);
    // }
    ...
    /**
     * All Elfs start out at the same age. Set the newbie's racial starting age,
     * and call the super method to set the current age in seconds. Each age
     * value is a pre-defined constant, and used for trait adjustment as Person
     * changes age brackets.
     * 
     * @return current starting age of Elf in years
     */
    public long initAge() {
        return super.initAge((long) _elfAgeBracket[0]);
    }

    /**
     * Initalize the height of the Elf with a range of plus/minus 16% of
     * average. Save the value as a base class attribute.
     * 
     * @return height of Elf in inches
     * @throws ChronosException
     *             if the defined Elf heights are invalid
     */
    ...
    /**
     * Initialize the weight of the Elf with a range of plus/minus 16% of
     * average. Save the value as a base class attribute.
     * 
     * @return weight of Elf in pounds
     */
    ...
    /**
     * Set the raclai language for the Elf
     * 
     * @return null
     */
    ...
    public class MockElf {
        ...
        public AttributeList adjustTraitsForRace(AttributeList traits)
                throws ApplicationException {
            return Elf.this.adjTraitsForRace(traits);
        }
        ...
        // skillList)
        // {
        // return Elf.this.assignRacialSkills(skillList);
        // }

        /**
         * Calculate the height of the Elf with a range of plus/minus 16% of
         * average. Save the value as a base class attribute.
         * 
         * @return height of Elf in inches
         */
        public int calcHeight() {
            return Elf.this.initHeight();
        }

        /**
         * Calculate the weight of the Elf with a range of plus/minus 16% of
         * average. Save the value as a base class attribute.
         * 
         * @return weight of Elf in pounds
         */
        public int calcWeight() {
            return Elf.this.initWeight();
        }

        /** Return the race name */
        public String getRaceName() {
            return Elf.this.getRaceName();
        }

        /*
         * Test current age and age bracket
         * 
         * @return currentAge of Elf in seconds
         */
        public long initAge() {
            return Elf.this.initAge();
        }
        ...
        public String initHeightDescriptor(double height, String gender) {
            setGender(gender);
            return Elf.this.initHeightDescriptor(height);
        }
